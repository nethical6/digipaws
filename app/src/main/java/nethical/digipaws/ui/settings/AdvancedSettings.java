...
public class AdvancedSettings extends PreferenceFragmentCompat
  ...
  public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
    if (key.equals("disable_uninstall")) {
      ...
      if (isSwitchOn) {
        // Switch is turned on
        devicePolicyManager =
            (DevicePolicyManager) requireContext().getSystemService(Context.DEVICE_POLICY_SERVICE);
        ...
        if (!devicePolicyManager.isAdminActive(deviceAdminReceiver)) {
          ...
          intent.putExtra(
              DevicePolicyManager.EXTRA_ADD_EXPLANATION, "Enable device admin to protect app");
          result(10001, REQUEST_CODE_ENABLE_ADMIN,intent);
        }
      } else {
  ...
  public ActivityResult result(int requestCode, int resultCode, @Nullable Intent data) {
    ...
    if (requestCode == REQUEST_CODE_ENABLE_ADMIN) {
      if (resultCode == Activity.RESULT_OK) {
        ...
        Toast.makeText(requireContext(), "Failed to enable device admin", Toast.LENGTH_SHORT)
            .show();
      }
    }
    return result(requestCode, REQUEST_CODE_ENABLE_ADMIN, data);
  }
}
