...
public class ContentBlockerSettings extends SettingsFragment {
	...
	public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
		...
		loader.startSearch(ListApps.getPackageNames(requireContext()), () -> {
	...
	public void loadScreen() {
		...
		for (String packageName : fullAppPacks) {
			...
			if (packageMap != null) {
				...
				getPreferenceScreen().addPreference(category); // Add the category to the preference screen
				...
				for (Map.Entry<String, BlockableView> entry : packageMap.entrySet()) {
					...
					SwitchPreferenceCompat switchPreference = new SwitchPreferenceCompat(requireContext());
					switchPreference.setTitle(entry.getValue().getTitle());
					switchPreference.setSummary(entry.getValue().getDescription()); // Set the summary for the switch
					switchPreference.setKey(packageName + ":id/" + entry.getValue().getViewId()); // Set a unique key for the switch
					...
					switchPreference.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
						...
						public boolean onPreferenceChange(Preference preference, Object newValue) {
							...
							if (isChecked) {
								...
								saveMapToPreferences(requireContext(), packageName, packageMap);
	...
	private static Map<String, BlockableView> getPackageMap(SharedPreferences preferences, String packageName) {
		String json = preferences.getString(packageName, null);
		if (json != null) {
			// Convert JSON string back to Map<String, Boolean>
			Gson gson = new Gson();
			Type type = new TypeToken<Map<String, BlockableView>>() {
			}.getType();
	...
	public static void saveMapToPreferences(Context context, String packageName, Map<String, BlockableView> map) {
