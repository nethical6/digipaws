...
public class FocusQuest extends Fragment {
    ...
    public void onResume() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            requireActivity().registerReceiver(timerUpdateReceiver, new IntentFilter("nethical.digipaws.TIMER_UPDATED"), Context.RECEIVER_NOT_EXPORTED);
        } else {
            requireActivity().registerReceiver(timerUpdateReceiver, new IntentFilter("nethical.digipaws.TIMER_UPDATED"), Context.RECEIVER_NOT_EXPORTED);
        }
        super.onResume();
    ...
    private MaterialAlertDialogBuilder makeQuestWarningDialog() {
        return new MaterialAlertDialogBuilder(requireContext())
                .setTitle("Focus Mode")
                .setMessage("Are you sure you want to proceed? You wont be able to access applications on your device for the next " + selectedFocusTime + " minutes. Remember that this quest cannot be stopped once started!")
