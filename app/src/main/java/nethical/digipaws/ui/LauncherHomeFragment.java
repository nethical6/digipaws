...
public class LauncherHomeFragment extends Fragment {
	...
	public void checkSurvival() {
		if (SurvivalModeConfig.isEnabled(requireContext())) {
			...
			info.setText("Your device has been locked until the clock reaches " + survivalModeEndTime
	...
	public void checkAudio() {
		AudioManager manager = (AudioManager) requireContext().getSystemService(requireContext().AUDIO_SERVICE);
	...
	public void onViewCreated(View arg0, Bundle arg1) {
		...
		mmtimeTextView.setTypeface(Typeface.createFromAsset(requireContext().getAssets(), "regulartime.ttf"),
				Typeface.BOLD);
		hhtimeTextView.setTypeface(Typeface.createFromAsset(requireContext().getAssets(), "regulartime.ttf"),
				Typeface.BOLD);
		...
		gestureDetector = new GestureDetector(requireContext(), new GestureDetector.SimpleOnGestureListener() {
			...
			public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
				...
				if (isSurvivalEnabled) {
					showToast("Your device will be unlocked after " + survivalModeEndTime + " hours.");
					return true;
				}
				...
				if (Math.abs(diffY) > SWIPE_THRESHOLD) {
					if (diffY < 0) {
						...
						requireActivity().getSupportFragmentManager()
								...
								.beginTransaction().setCustomAnimations(R.anim.fade_enter, R.anim.fade_exit) // Use 0 for no exit animation

								.replace(R.id.fragment_container, launcherAppsFragment).addToBackStack(null).commit();
						return true;
					}
				}
				return false;
	...
	private void generateStats(){
		...
		String formattedValue =( "STREAK: "+ String.valueOf(streak) + "\n" +
								"Times Content Blocked: " + String.valueOf(viewBlocked) + "\n" +
								"Times Keyword Blocked: " + String.valueOf(viewBlocked) + "\n" );
