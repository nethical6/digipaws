    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        NotificationCompat.Builder notification =
                new NotificationCompat.Builder(
                                getApplicationContext(), Constants.NOTIFICATION_CHANNEL)
                        ...
                        () -> {
                            SharedPreferences preferences =
                                    context.getSharedPreferences(
                                            Constants.LOC_QUEST_DATA_PREF, Context.MODE_PRIVATE);
                            radarLocation.setLatitude(
                                    preferences.getFloat(Constants.LOC_QUEST_RADAR_LAT_PREF, 0.0f));
                            radarLocation.setLongitude(
                                    preferences.getFloat(Constants.LOC_QUEST_RADAR_LON_PREF, 0.0f));
                                            ...
                                            if (location != null) {
                                                ...
                                                if (distance > Constants.LOC_QUEST_RADAR_RADIUS) {
                                                    ...
                                                    DigiUtils.sendNotification(
                                                            context,
                                                            "Quest Completed!!!",
                                                            "Survival Mode has been disabled");
