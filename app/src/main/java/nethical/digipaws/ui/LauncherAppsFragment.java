...
public class LauncherAppsFragment extends Fragment implements FragmentChangeListener {
	...
	private void resetStatusBarColor() {
		...
		window.setBackgroundDrawableResource(R.color.app_background_color); // Clear existing background (optional)
	...
	public void onFragmentChange(Fragment fragment) {
		if (getActivity() != null) {
			getParentFragmentManager().beginTransaction().setCustomAnimations(R.anim.fade_enter, R.anim.fade_exit) // Use 0 for no exit animation
	...
	private void initializeAppList() {
		...
		ListApps.getUserAccessibleApps(requireContext(), appList -> {
			allAppsList = appList;

			CustomLinearLayoutManager layoutManager = new CustomLinearLayoutManager(requireContext());
			appsRecyclerView.setLayoutManager(layoutManager);
			layoutManager.setRecyclerView(appsRecyclerView); // Assign the RecyclerView to the layout manager

			appAdapter = new AppAdapter(requireContext(), appList, this);
			appsRecyclerView.setAdapter(appAdapter);
		});

		appsRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
			boolean onTop = false;
			...
			public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
				...
				case RecyclerView.SCROLL_STATE_DRAGGING:
					...
					if (onTop) {
						...
						KeyboardUtils.hideKeyboard(requireContext(), searchApp); // 'this' is the context
				...
				case RecyclerView.SCROLL_STATE_IDLE:
					...
					if (!recyclerView.canScrollVertically(1)) {
						...
						if (!onTop) {
							// Assuming 'binding' is your view binding object
							// and 'search' is the view you want to manipulate
							KeyboardUtils.showKeyboard(requireContext(), searchApp); // 'this' is the context
	...
	public void toggleAppListVisibility() {
		...
		getParentFragmentManager().beginTransaction().setCustomAnimations(R.anim.fade_enter, R.anim.fade_exit) // Use 0 for no exit animation
				...
				.replace(R.id.fragment_container, launcherHomeFragment).addToBackStack(null).commit();
	}
	...
	public class CustomLinearLayoutManager extends LinearLayoutManager {
		...
		public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
			...
			if (overScroll < -10 && recyclerView != null
					&& recyclerView.getScrollState() == RecyclerView.SCROLL_STATE_DRAGGING) {
				// move back to home activity when recycler is overscrolled towards the bottom direction
